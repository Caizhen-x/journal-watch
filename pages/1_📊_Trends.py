"""Trend dashboard — topic/method evolution over time, journal x topic heatmap.

Inspired by paulgp.com/econlit-pipeline/ but built on our existing classified-paper DB.
"""
import json
import sqlite3
...
def load_classified_papers() -> pd.DataFrame:
    """Return one row per paper × tag pair (long format), restricted to classified papers."""
    ...
    for r in rows:
        ...
        for t in json.loads(r["topics_json"]):
            records.append({
                "doi": r["doi"], "journal": r["journal_code"], "year": int(year),
                "kind": "topic", "tag": t, "relevance": r["relevance"],
            })
        for m in json.loads(r["methods_json"]):
            records.append({
                "doi": r["doi"], "journal": r["journal_code"], "year": int(year),
                "kind": "method", "tag": m, "relevance": r["relevance"],
            })
...
def load_paper_summaries() -> pd.DataFrame:
    """One row per classified paper (for top-relevance reels)."""
    if not DB_PATH.exists():
        return pd.DataFrame()
    c = sqlite3.connect(DB_PATH)
    c.row_factory = sqlite3.Row
...
def main():
    ...
    if yr_min == yr_max:
        ...
        yr_range = st.slider(
            ...
            value=(max(yr_min, yr_max - 9), yr_max),
        )
    ...
    with tab_topic:
        ...
        selected = st.multiselect(
            "Topics to show (default: all)",
            all_topics, default=all_topics, key="trend_topics",
        )
        plot = topic_df[topic_df["topic_label"].isin(selected)]
        agg = (plot.groupby(["year", "topic_label"])["doi"]
        ...
        if chart_kind == "Stacked area":
            ...
            chart = alt.Chart(agg).mark_area().encode(
                x=alt.X("year:O", title="Year"),
                y=alt.Y("papers:Q", title="Share of tagged papers", stack="normalize"),
                color=alt.Color("topic_label:N", title="Topic"),
                tooltip=["year", "topic_label", "papers"],
            )
    ...
    with tab_method:
        ...
        # Default selection: top N most-used methods in window
        ...
        selected_m = st.multiselect(
            "Methods to show",
            sorted(method_df["method_label"].unique()),
            default=default_methods, key="trend_methods",
        )
        plot_m = method_df[method_df["method_label"].isin(selected_m)]
        agg_m = (plot_m.groupby(["year", "method_label"])["doi"]
                       .nunique().reset_index(name="papers"))
        chart_m = alt.Chart(agg_m).mark_line(point=True).encode(
            x=alt.X("year:O", title="Year"),
            y=alt.Y("papers:Q", title="Papers using method"),
            color=alt.Color("method_label:N", title="Method"),
            tooltip=["year", "method_label", "papers"],
        ).properties(height=420)
        st.altair_chart(chart_m, use_container_width=True)

    # -------- Journal x topic heatmap --------
    with tab_heatmap:
        ...
        heat = (topic_df.groupby(["journal", "topic_label"])["doi"]
                        .nunique().reset_index(name="papers"))
        chart_h = alt.Chart(heat).mark_rect().encode(
            x=alt.X("topic_label:N", title="Topic", axis=alt.Axis(labelAngle=-40)),
            y=alt.Y("journal:N", title="Journal"),
            color=alt.Color("papers:Q", title="Papers", scale=alt.Scale(scheme="blues")),
            tooltip=["journal", "topic_label", "papers"],
        ).properties(height=520)
        st.altair_chart(chart_h, use_container_width=True)

    # -------- Top relevance per year --------
    with tab_top:
        st.caption(f"Highest-relevance paper of each year in {yr_range[0]}–{yr_range[1]}.")
        if summaries.empty:
            st.info("No data in window.")
        else:
            top_by_year = summaries.sort_values(["year", "relevance"], ascending=[True, False]) \
                                   .groupby("year").head(3)
            for year, group in top_by_year.groupby("year", sort=False):
                st.markdown(f"### {year}")
                for _, r in group.iterrows():
                    st.markdown(
                        f"- **rel {r['relevance']}/10** · [{r['title']}](https://doi.org/{r['doi']}) "
                        f"— *{r['journal']}*, {r['pub_date']} — {r['authors']}"
                    )

    # -------- Per-journal volume --------
    with tab_journal:
        st.caption("Classified paper volume per journal per year. Useful to spot coverage gaps.")
        vol = (summaries.groupby(["year", "journal"])["doi"]
                        .nunique().reset_index(name="papers"))
        chart_v = alt.Chart(vol).mark_bar().encode(
            x=alt.X("year:O"),
            y=alt.Y("papers:Q", title="Papers"),
            color=alt.Color("journal:N", title="Journal"),
            tooltip=["journal", "year", "papers"],
        ).properties(height=420)
